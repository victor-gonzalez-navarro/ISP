...
def train_lstm_regressor(news, test_size, window_size, word2vec_model):
...
def main():
    ...
    for window_size in WINDOWS_SIZES:
        model, history = train_lstm_regressor(news, test_size, window_size, word2vec_model)
        model.save_weights(str(MODELS_PATH / 'news_model_lstm_regressor_ws{:d}.h5'.format(window_size)))
        models.append(model)
